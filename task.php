...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    if(isset($_POST['task']) && !empty(trim($_POST['task']))) {
        ...
        $tasks[$_POST['toggle']]['completed'] =!$tasks[$_POST['toggle']]['completed'];
        saveTasks($tasks);
        header('Location: ' . $_SERVER['PHP_SELF']);
        exit();
    }
                ...
                <form method="POST">
                    <div class="row">
                        <div class="column column-75">
                            <input type="text" name="task" placeholder="Type your new task here" required>
                        </div>
                        ...
                        <li class="task-item">
                            <form method="POST" style="flex-grow: 1;">
                                ...
                                <button type="submit" style="border: none; background: none; cursor: pointer; text-align: left; width: 100%;">
                                    <span class="task <?= $task['completed'] ? 'task-done' : '' ?>">
